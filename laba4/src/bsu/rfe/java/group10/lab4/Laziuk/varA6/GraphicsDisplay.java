package bsu.rfe.java.group10.lab4.Laziuk.varA6;
...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f, new float[] {20, 10, 10, 10, 10, 10, 10, 10, 20, 10, 10, 10}, 0.0f);
        // Перо для рисования осей координат
        axisStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, null, 0.0f);
        // Перо для рисования контуров маркеров
        markerStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, null, 0.0f);
        // Перо для рисования узора
    ...
    public void showGraphics(Double[][] graphicsData) {  // при выборе бинарного файла сразу отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            Point2D.Double center = xyToPoint(point[0], point[1]);
             ...
             if (isEven(point[1]))  // если у четный - точка зеленая
            ...
            canvas.draw(new Line2D.Double(corner.x - 5.5, corner.y, corner.x - 5.5, corner.y - 10));
            canvas.draw(new Line2D.Double(corner.x, corner.y - 5.5, corner.x - 11, corner.y - 5.5));
            ...
            //canvas.draw(new Ellipse2D.Double(15, 15, 0.5, 0.5));
        }
    }
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if(minY <= 0.0 && maxY >= 0.0 && minX <= 0.0 && maxX >= 0.0) {
            ...
            canvas.drawString("0", (float)(labelPos.getX() + bounds.getWidth()), (float)(labelPos.getY() - bounds.getY()));
        }

        // добавить штрих на оси Х на точке (1, 0)
        canvas.setColor(Color.BLACK);
        GeneralPath path = new GeneralPath();
        Point2D.Double linepos = xyToPoint(1,0);
        Rectangle2D x1 = axisFont.getStringBounds("1", context);
        canvas.drawString("1", (float) (linepos.getX()-
                x1.getWidth() + 10), (float) (linepos.getY() + x1.getY() + 70) );
        path.moveTo(linepos.getX(),linepos.getY());
        path.lineTo(linepos.getX(),linepos.getY()+10);
        path.lineTo(linepos.getX(),linepos.getY()-20);
        path.closePath();
        canvas.draw(path);
    }
